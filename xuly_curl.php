...
//      $this->options[CURLOPT_HEADER] = $val;
 // }
    ...
    public function trinhduyet($trinhduyet)
    {
        $this->options[CURLOPT_USERAGENT] = $trinhduyet;
    }

    public function guipost($postdata)
    {
    ...
    public function xemtrang($url)
    {
        $ch = curl_init($url);
        curl_setopt_array($ch, $this->options);
        $this->xuatnoidung = curl_exec($ch);
...
$config['cookie_file'] = $dir . '/cookies/' . rand(1000000000,9999999999999999).'.txt';


if (!file_exists($dir . '/cookies/index.html'))
{
    $f = fopen($dir . '/cookies/index.html', 'w');
    fwrite($f, 'CHECKER PRO');
    fclose($f);
}
